...
total.forEach(function(item, i) {
  ...
  let active_li = item.querySelector("li > .active");
  moving_div.style.padding = "0px";
  if (!active_li || Array.from(item.querySelectorAll("li")).indexOf(active_li) === 0) {
        ...
        nav_link.onclick = function() {
          ...
          if (item.classList.contains("flex-column")) {
            for (let j = 1; j <= nodes.indexOf(li); j++) {
              sum += item.querySelector("li:nth-child(" + j + ")").offsetHeight - 0.4;
              moving_div.style.height = item.querySelector("li:nth-child(" + j + ")").offsetHeight;
            }
            moving_div.style.transform = "translate3d(0px," + sum + "px, 0px)";
          } else {
            for (let j = 1; j <= nodes.indexOf(li); j++) {
              sum += item.querySelector("li:nth-child(" + j + ")").offsetWidth - 0.4;
            }
...
function darkMode(el) {
  ...
  if (!el.getAttribute("checked")) {
    ...
    document.querySelector("#sidebar-theme-select").children[0].click(); // Even the sidebar change of theme when theme changing, but the change wasn"t saved in a cookie.
  }
};
